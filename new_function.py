...
def combine_boxes(boxes, box_size):
    removelist = []
    w_dif = 10
    l_dif = 10
    for i in range(len(boxes)):
        for j in range(i + 1, len(boxes)):
                if int(boxes[i][-1][0] - box_size[0]) < w_dif and int(boxes[i][-1][1] - box_size[1]) < l_dif :
...
def best_box_picker(boxes, box_limit, box_size):
    # Z -> X -> Y
    # Y -> X -> Z
    new_list = []
    last_box_h = 0
    ...
    for index, box_list in enumerate(new_combine_list):
        ...
        if boxes != []:
            ...
            best_box = []  
    final_list = combine_boxes(final_list, box_size)      
    return final_list